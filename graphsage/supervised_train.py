...
def save_val_embeddings(sess, model, minibatch_iter, size, out_dir, mod=""):
    ...
    while not finished:
        ...
        iter_num += 1
        outs_val = sess.run([model.preds, model.outputs1], 
                            feed_dict=feed_dict_val)
    ...
    for epoch in range(FLAGS.epochs): 
        ...
        while not minibatch.end():
            ...
            if total_steps % FLAGS.print_every == 0:
                ...
                y_pred[y_pred > 0.1] = 1
                y_pred[y_pred <= 0.1] = 0
                print(metrics.confusion_matrix(labels[:,1], y_pred))
                print("Accuracy=", 1.0 * np.sum(np.equal(labels[:,1], y_pred))/labels.shape[0])
